  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
        ...
        position += substr_len;
        // printf("match token = \"%s\", tokens[%d].type = %d, tokens[%d].str=
        // \"%s\"\n", 				rules[i].regex, count-1,
        // tokens[count-1].type, count-1,
        // tokens[count-1].str);
  ...
  if (p > q) {
    ...
    if (tokens[op].type == TK_MINUS) {
      ...
      case TK_LOGIC_AND:
        ...
        break;
      default:
        panic("Operator symbol \"%s\"is correct.", tokens[op].str);
      }
    }
  }
...
void p() {
  ...
  char *e = (char *)malloc(4096 * sizeof(char) + 1);
  printf("请输入表达式：");
  scan = scanf("%[^\n]", e);
  if (scan <= 0) {
