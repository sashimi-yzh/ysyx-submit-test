...
static void init_difftest0(void *dut_r, size_t img_size) {
  ...
  difftest_sync(regs, dut_mem, img_size); // now img_size is more than the memory to sync
#ifndef PROG_IN_FLASH
  regs[NR_REGS] = FLASH_BASE;
  uint32_t *p;
  for (p = (uint32_t *)flash; *p != 0; p ++);
  uint32_t inst_size = (uint8_t *)p - flash;
  difftest_sync(regs, flash, inst_size);
#endif
}
...
int main(int argc, char *argv[]) {
  ...
  ((uint32_t *)&flash)[0] = 0x50000297; // auipc t0, 0x50000
  ((uint32_t *)&flash)[1] = 0x00028067; // jr t0
