...
class SimDMEM extends BlackBox with HasBlackBoxInline with NPCParameters {
  ...
  setInline("SimDMEM.v",
   s"""module SimDMEM(
      ...
      |reg [${XLEN-1}:0] mem [0:2**WORD_BITS-1];
      |initial begin
      |  $$readmemh("prog.bin", mem);
      |end
      |
      |wire [${XLEN-1}:0] wmask_full;
      |genvar i;
      |generate
      |  for (i = 0; i < ${XLEN/8}; i = i + 1) begin
      |    assign wmask_full[i*8+7:i*8] = {8{wmask[i]}};
      |  end
      |endgenerate
      |
      |always @(*) begin
      |  rdata = ren ? mem[raddr[WORD_BITS-1+2:2]] : 0;
      |  //if (ren) $$display("raddr = 0x%x, rdata = 0x%x", raddr, rdata);
      |end
      |always @(posedge clock) begin
      |  if (wen) begin
      |    if (waddr == 32'h10000000) $$write("%c", wdata[7:0]);
      |    else mem[waddr[WORD_BITS-1+2:2]] <= (wdata & wmask_full) | (mem[waddr[WORD_BITS-1+2:2]] & ~wmask_full);
      |    //$$display("waddr = 0x%x, wdata = 0x%x, wmask = 0x%x", waddr, wdata, wmask);
      |  end
      |end
      |`else
...
class Flash extends NPCModule {
  val io = IO(Flipped(new MemRWPortBundle))
  val insts = List(0x50000297L, // auipc t0, 0x50000
                   0x00028067L) // jr t0
  val inRange = io.raddr >= 0x30000000.U && io.raddr < 0x30000008.U
  val rom = VecInit(insts.map(x => x.U(32.W)))
  io.rdata := Mux(io.ren && inRange, rom(io.raddr(2)), 0.U)
}
...
class AXI4Flash extends Module {
  val io = IO(Flipped(new AXI4))
  val axi4memctrl = Module(new AXI4MemCtrl)
  val flash = Module(new Flash)
  axi4memctrl.io.axi <> io
  flash.io <> axi4memctrl.io.dmem
}
