...
class NPCTrap extends BlackBox with HasBlackBoxInline with NPCParameters {
  ...
  setInline("NPCTrap.v",
   s"""module NPCTrap(
      ...
      |    end
      |    else begin;
      |      $$display("HIT BAD TRAP with code = %d", code);
      |      $$fatal;
      |    end
      |  end
      |end
...
class NPC extends NPCModule {
  val io = IO(new Bundle{
    val master    = new AXI4(idBits = 4)
    val slave     = Flipped(new AXI4(idBits = 4))
  ...
  assert(!(io.master.r.fire && io.master.r.bits.resp =/= AXI4Parameters.RESP_OKAY))
  assert(!(io.master.b.fire && io.master.b.bits.resp =/= AXI4Parameters.RESP_OKAY))
...
class Top extends NPCModule {
  ...
  if (Global.mode == "sim") {
    ...
    xbar.io.in <> npc.io.master
    flash.io <> xbar.io.out(0)
    mem.io <> xbar.io.out(1)
