//final version by yangjiaxin, 2025.09.23
...
module ysyx_23060229_IFU(
	...
	//wire exception_quest_IFU_pred = |pc[1:0] | (|pc[31:24] == 0);
	//wire exception_quest_IFU = |pc_jump[1:0] | (|pc_jump[31:24] == 0);
	...
	ysyx_23060229_icache myicache (clock, stall_quest_fencei, wen, index, offset, valid_in, tag_in, data_in, valid_out, tag_out, data_out);
	...
	//assign rready = (state == Idle & ~hit & ~stall_quest_fencei & ~stall_quest_exception_IFU) || (state == Wait_Rvalid);
	//assign arvalid = (state == Idle) & (~stall_quest_fencei) & (~stall_quest_exception_IFU);
	...
	wire [31:0] op = (is_branch & pred_jump) ? imm_branch :
					(is_jal) ? imm_jal : 4;
	//assign wrong_pred = (check_quest & (pc != pc_jump));
	//assign wrong_pred = (check_quest & (pc != pc_jump)) | exception_quest_IFU;
	...
	always @(posedge clock) begin
		...
		/*else if(stall_quest_exception_IFU & state != Wait_Rvalid & state != Idle) begin
			state <= Idle;
			pc <= mtvec; inst <= 0;
			wen <= 0;
		end*/
			...
			case(state)
				...
				Idle: begin
					check_assert <= 0;
					//if(stall_quest_exception_IFU) begin//不影响此次访存
					//	to_reset <= 1;
						//state <= Wait_IDU_Ready;
					//end

					//if(exception_quest_IFU_pred) begin//pc一旦发生异常则不会访存
					//	state <= Wait_IDU_Ready;//疑似出现异常,即推测的跳转目的地址异常
						//实际上可以继续访存直至此次结束,因为araddr始终是对齐的形式
						//只要to_reset = 1即可确保下游不会错误取走指令
					//end
				...
				Wait_Rvalid: begin
                    ...
                    `endif
					//软复位, 即reset信号不为全局复位信号(AXI4Xbar不会被复位)
					//if(stall_quest_exception_IFU)//延迟的复位(等待访存结束才复位)
                    //    to_reset <= 1;
...
module ysyx_23060229_IDU(
	...
	input stall_quest_exception_IDU,
	...
	//output reg exception_quest_IFU,//托管发出IFU的异常
	output reg exception_quest_IDU
);	
	...
	always @(posedge clk) begin
		if(rst | stall_quest_exception_IDU) begin
			exception_quest_IDU <= 0;
			...
			//`ifdef verilator  $display("IDU resetting ..."); `endif
		end
			...
			case(state)
				Wait_IFU_Valid: begin
					if(validFromIFU) begin//确保IFU预测正确时才进行状态转换
						...
						`ifdef verilator
							`ifdef ysyx_23060229_CONFIG_ITRACE
					   			fd = $fopen("itrace.log", "a+");
								$fdisplay(fd, "%x", pc);
								$fclose(fd);
							`endif
						`endif
...
module ysyx_23060229_EXU(
	...
	input stall_quest_exception_EXU,
	...
	always @(posedge clk) begin
		if(rst | stall_quest_exception_EXU) begin
			...
			case(state)
				Wait_IDU_Valid: begin
					...
					if(validFromIDU) begin
            			...
            			//    `ysyx_23060229_Jump_CS: begin//ECALL的预测结果大概率错误,因而需发出check_quest
								//check_quest <= 1;
								//pc_jump <= srccs;
						//	end
...
module ysyx_23060229_LSU(
	...
	input stall_quest_exception_LSU,
	...
	output exception_quest_IFU,//托管发出IFU异常请求
	output exception_quest_LSU
	...
	//这些错误本质上由EXU报出
	wire excp_la_misaligned = 	((typ == `ysyx_23060229_I_LW) & |dest_csreg_mem[1:0]) ||
								((typ == `ysyx_23060229_I_LH) & dest_csreg_mem[0]) ||
								((typ == `ysyx_23060229_I_LHU) & dest_csreg_mem[0]);
	wire excp_sa_misaligned = 	((typ == `ysyx_23060229_S_SW) & |dest_csreg_mem[1:0]) ||
								((typ == `ysyx_23060229_S_SH) & dest_csreg_mem[0]);
	wire excp_laccess_fault = 	((flag == `ysyx_23060229_ReadMem) & (|dest_csreg_mem[31:24] == 0));//假设的非法地址: 0x00??_????,仅供测试
	wire excp_saccess_fault = 	((flag == `ysyx_23060229_WriteMem) & (|dest_csreg_mem[31:24] == 0));//假设的非法地址: 0x00??_????,仅供测试

	assign exception_quest_LSU = excp_la_misaligned | excp_sa_misaligned | excp_laccess_fault | excp_saccess_fault;
	
	assign exception_quest_IFU = (|pc_jump[1:0]) | (|pc_jump[31:24] == 0);//假设的非法地址,仅供测试
	...
	assign arvalid = (state == Wait_EXU_Valid && flag == `ysyx_23060229_ReadMem && validFromEXU && ~exception_quest_LSU);
	assign rready = (state == Wait_EXU_Valid && flag == `ysyx_23060229_ReadMem && validFromEXU && ~exception_quest_LSU) || (state == Wait_Rvalid);
	...
	assign awvalid = (state == Wait_EXU_Valid && flag == `ysyx_23060229_WriteMem && validFromEXU && ~exception_quest_LSU) || (state == Wait_Awready);
	assign wvalid =  (state == Wait_EXU_Valid && flag == `ysyx_23060229_WriteMem && validFromEXU && ~exception_quest_LSU) || (state == Wait_Wready);
	assign bready = (state == Wait_EXU_Valid && flag == `ysyx_23060229_WriteMem && validFromEXU && ~exception_quest_LSU) || (state == Wait_Awready) || (state == Wait_Wready) 
					|| (state == Wait_Bvalid);
	...
	always @(posedge clock) begin
		...
		if(rst | stall_quest_exception_LSU) begin
			...
			case(state)
				...
				if(validFromEXU) begin	
					...
					if(flag == `ysyx_23060229_Jump_J) begin//仅写reg
						...
						wen_csreg <= 0;
						if(|pc_jump[31:24] == 0) begin
							wen_csreg <= 8'b11000001;//excp.no = 0x1
						end
						if(|pc_jump[1:0]) begin
							`ifdef verilator
                                $display("Exception: Inst Address Misaligned in IFU! Error pc: %x", pc);
                            `endif
							wen_csreg <= 8'b11000000;//excp.no = 0x0
						end
                        //`ifdef simulation one_inst_done <= 1; `endif
                    end
					if(flag == `ysyx_23060229_Jump_CS) begin//仅写csreg
						wen_reg <= 0;
						state <= Wait_EXU_Valid;
						waddr_csreg <= `ysyx_23060229_MEPC; wdata_csreg <= pc;
					...
					if(flag == `ysyx_23060229_ReadMem) begin//读mem+写reg
						...
						if(excp_la_misaligned) begin
							`ifdef verilator
								$display("Exception: Load Address Misaligned in LSU! Error pc: %x", pc);
							`endif
							wen_csreg <= 8'b11000100;//excp.no = 0x4
						end
						else if(excp_laccess_fault) begin
                            `ifdef verilator
                                $display("Exception: Load Access Fault in LSU! Error pc: %x", pc);
                            `endif
                            wen_csreg <= 8'b11000101;//excp.no = 0x5
                        end
					...
					if(flag == `ysyx_23060229_WriteMem) begin//写mem
						if(excp_sa_misaligned) begin
							`ifdef verilator
								$display("Exception: Store Address Misaligned in LSU! Error pc: %x", pc);
							`endif
							wen_reg <= 0; wen_csreg <= 8'b11000110;//excp.no = 0x6
							waddr_csreg <= `ysyx_23060229_MEPC; wdata_csreg <= pc;
							state <= Wait_EXU_Valid;
						end
						else if(excp_saccess_fault) begin
                            `ifdef verilator
                                $display("Exception: Store Access Fault in LSU! Error pc: %x", pc);
                            `endif
                            wen_reg <= 0; wen_csreg <= 8'b11000111;//excp.no = 0x7
                            waddr_csreg <= `ysyx_23060229_MEPC; wdata_csreg <= pc;
                            state <= Wait_EXU_Valid;
                        end
...
//-----------------------------------------V3:ExceptionCtl-----------------------------------------//
module ysyx_23060229_ExceptionCtl(
	input clk,
	input rst,
	input exception_quest_IFU,
	input exception_quest_IDU,
	input exception_quest_LSU,
	input [31:0] exception_mepc_IFU,
	input [31:0] exception_mepc_IDU,
	input [31:0] exception_mepc_LSU,//异常发生时生成的epc
	input [31:0] mepc,//来自CSReg的epc
	input excp_written,//来自CSReg
	output stall_quest_exception_IFU,
	output stall_quest_exception_IDU,
	output stall_quest_exception_EXU,
	output stall_quest_exception_LSU
);


	parameter 	Idle = 2'b00,
				service_for_LSU = 2'b01,
				service_for_IDU = 2'b10,
				service_for_IFU = 2'b11;
	...
	always @(posedge clk) begin
		if(rst)
			...
			case(state)
				Idle: begin
					/*if(exception_quest_LSU) begin
						state <= service_for_LSU; mepc_tmp <= exception_mepc_LSU;
					end
					else if(exception_quest_IDU) begin
						state <= service_for_IDU; mepc_tmp <= exception_mepc_IDU;
					end
					else if(exception_quest_IFU) begin
						state <= service_for_IFU; mepc_tmp <= exception_mepc_IFU;
					end*/
					casez({exception_quest_LSU, exception_quest_IDU, exception_quest_IFU})
                        3'b1??: begin
                            state <= service_for_LSU; mepc_tmp <= exception_mepc_LSU;
                        end
                        3'b01?: begin
				...
				service_for_LSU: begin//最高优先级
					if(mepc_tmp == mepc & excp_written) begin
						state <= Idle;
					end
				end

				service_for_IDU: begin//次高优先级
					//if(exception_quest_LSU) begin//让位给LSU
					//	state <= service_for_LSU; mepc_tmp <= exception_mepc_LSU;
					//end
					if(mepc_tmp == mepc & excp_written) begin
						state <= Idle;
				...
				service_for_IFU: begin//最低优先级
					//if(exception_quest_LSU) begin//优先让位给LSU
					//	state <= service_for_LSU; mepc_tmp <= exception_mepc_LSU;
					//end
					//else if(exception_quest_IDU) begin//次优先让位给IDU
					//	state <= service_for_IDU; mepc_tmp <= exception_mepc_IDU;
					//end
					if(mepc_tmp == mepc & excp_written) begin
						state <= Idle;
	...
	assign stall_quest_exception_IFU = |state;
	assign stall_quest_exception_IDU = (|state) | excp_written;
	//assign stall_quest_exception_IFU = (state == service_for_LSU) | (state == service_for_IDU) | (state == service_for_IFU);
	//assign stall_quest_exception_IDU = (state == service_for_LSU) | (state == service_for_IDU) | (state == service_for_IFU) | excp_written;
	assign stall_quest_exception_EXU = (state == service_for_LSU) | excp_written;
	assign stall_quest_exception_LSU = (state == service_for_LSU) | excp_written;
...
module ysyx_23060229(
	...
	wire exception_quest_IFU;//托管发出IFU异常请求
	wire exception_quest_LSU;
    ...
    //与ExceptionCtl相关的接口
	//`ifdef simulation
		wire [31:0] exception_mepc_IFU = EX_LS_pc;//由LSU托管
		wire [31:0] exception_mepc_LSU = EX_LS_pc;
	//`else
	//	wire exception_mepc_IFU = EX_LS_pc;//由LSU托管
	//	wire exception_mepc_LSU = EX_LS_pc;
	//`endif
	wire [31:0] exception_mepc_IDU = ID_EX_pc;
	wire stall_quest_exception_IFU;
	wire stall_quest_exception_IDU;
	wire stall_quest_exception_EXU;
	wire stall_quest_exception_LSU;
	/*------------------------------------------------------------------------------*/


	ysyx_23060229_IFU myifu		(clock, reset, check_quest, stall_quest_fencei, pc_jump, check_assert,
								stall_quest_exception_IFU, mtvec,
	...
	ysyx_23060229_LSU	mylsu	(clock, reset, EXU_valid_LSU, LSU_ready_EXU,
								...
								exception_quest_IFU, exception_quest_LSU
								);
	...
	ysyx_23060229_ExceptionCtl myec		(clock, reset, exception_quest_IFU, exception_quest_IDU, exception_quest_LSU,
										exception_mepc_IFU, exception_mepc_IDU, exception_mepc_LSU, mepc, excp_written,
										stall_quest_exception_IFU, stall_quest_exception_IDU, stall_quest_exception_EXU, stall_quest_exception_LSU
										);
