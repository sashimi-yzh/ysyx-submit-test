...
def_EHelper(add) {
  ...
  if (need_update_eflags) {
    ...
    rtl_set_lazycc(s, res, NULL, NULL, LAZYCC_ADD, s->isa.width);
#else
    rtl_update_ZFSF(s, res, s->isa.width);
    if (s->isa.width != 4) {
      rtl_andi(s, res, res, 0xffffffffu >> ((4 - s->isa.width) * 8));
    }
    rtl_is_add_carry(s, s1, res, ddest);
    rtl_set_CF(s, s1);
    rtl_is_add_overflow(s, s1, res, ddest, dsrc1, s->isa.width);
    rtl_set_OF(s, s1);
#endif
  }
  rtl_wb(s, res);
...
def_EHelper(sub) {
  ...
  int need_update_eflags = MUXDEF(CONFIG_x86_CC_NONE, true, s->isa.flag_def != 0);
  if (need_update_eflags) {
    MUXDEF(CONFIG_x86_CC_LAZY, rtl_set_lazycc(s, ddest, dsrc1, NULL, LAZYCC_SUB, s->isa.width), res = s0);
  }
  ...
  if (need_update_eflags && !ISDEF(CONFIG_x86_CC_LAZY)) {
    ...
    rtl_set_CF(s, s1);
    rtl_is_sub_overflow(s, s1, res, ddest, dsrc1, s->isa.width);
    rtl_set_OF(s, s1);
  }
  rtl_wb(s, res);
}
...
#else
    rtl_update_ZFSF(s, ddest, s->isa.width);
    rtl_setrelopi(s, RELOP_EQ, s1, ddest, (0x1u << (s->isa.width * 8 - 1)) - 1);
    rtl_set_OF(s, s1);
#endif
  }
  rtl_wb(s, ddest);
...
def_EHelper(adc) {
  ...
  if (need_update_eflags) {
#ifdef CONFIG_x86_CC_LAZY
    rtl_set_lazycc(s, res, s0, NULL, LAZYCC_ADC, s->isa.width);
#else
    rtl_update_ZFSF(s, res, s->isa.width);
    rtl_is_add_overflow(s, s2, res, ddest, dsrc1, s->isa.width);
    rtl_set_OF(s, s2);
    if (s->isa.width != 4) {
      rtl_andi(s, res, res, 0xffffffffu >> ((4 - s->isa.width) * 8));
    }
...
def_EHelper(sbb) {
  ...
  if (need_update_eflags) {
#ifdef CONFIG_x86_CC_LAZY
    rtl_set_lazycc(s, res, NULL, NULL, LAZYCC_SBB, s->isa.width);
#else
    rtl_update_ZFSF(s, res, s->isa.width);
    rtl_is_sub_overflow(s, s2, res, ddest, dsrc1, s->isa.width);
    rtl_set_OF(s, s2);
...
def_EHelper(mul) {
  ...
  if (need_update_eflags) {
#ifdef CONFIG_x86_CC_LAZY
    rtl_set_lazycc(s, hi, NULL, NULL, LAZYCC_MUL, s->isa.width);
#else
