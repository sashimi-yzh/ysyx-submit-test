...
static word_t x86_inst_fetch(Decode *s, int len, bool advance_p_inst) {
  ...
  if (advance_p_inst) s->isa.p_inst += len;
  return ret_save;
}

static word_t get_inst(Decode *s) {
  return *(s->isa.p_inst - 1);
}

static word_t fetch_imm(Decode *s, int width) {
  return x86_inst_fetch(s, width, false);
}

static word_t fetch_simm(Decode *s, int width) {
#ifdef __ICS_EXPORT
  /* TODO: Use inst_fetch() to read `width' bytes of memory
   * pointed by 's->snpc'. Return the result as a signed immediate.
   */
  TODO();
  return 0;
#else
  word_t imm = x86_inst_fetch(s, width, false);
...
static void load_addr(Decode *s, ModR_M *m, Operand *rm) {
  ...
  if (m->R_M == R_ESP) {
    SIB sib;
    sib.val = x86_inst_fetch(s, 1, false);
    base_reg = sib.base;
    scale = sib.ss;
  ...
  if (disp_size != 0) {
    /* has disp */
    disp = x86_inst_fetch(s, disp_size, false);
...
/* Ob, Ov */
def_DopHelper(O) {
  s->isa.moff = x86_inst_fetch(s, 4, false);
  s->isa.mbase = rz;
  s->isa.midx = rz;
  op->preg = &op->val;
  op->type = OP_TYPE_MEM;
...
#define def_x86_INST_IDTABW(decode_fun, pattern, id, tab, w) \
  def_INST_raw(decode_fun, pattern, { \
      if (w != -1) s->isa.width = (w == 0 ? (s->isa.is_operand_size_16 ? 2 : 4) : w); \
      concat(decode_, id)(s, s->isa.width); \
      return concat(table_, tab)(s); \
    })

#undef def_INST_IDTABW
#define def_INST_IDTABW(pattern, id, tab, w) \
  def_x86_INST_IDTABW(pattern_decode, pattern, id, tab, w)

#undef def_hex_INST_IDTABW
#define def_hex_INST_IDTABW(pattern, id, tab, w) \
  def_x86_INST_IDTABW(pattern_decode_hex, pattern, id, tab, w)

def_THelper(gp1) {
  def_INST_TABW("?? 000 ???", add, -1);
  def_INST_TABW("?? 001 ???", or , -1);
  def_INST_TABW("?? 010 ???", adc, -1);
  def_INST_TABW("?? 011 ???", sbb, -1);
  def_INST_TABW("?? 100 ???", and, -1);
  def_INST_TABW("?? 101 ???", sub, -1);
  def_INST_TABW("?? 110 ???", xor, -1);
  def_INST_TABW("?? 111 ???", cmp, -1);
  return EXEC_ID_inv;
}

def_THelper(gp2) {
  def_INST_TABW("?? 000 ???", rol, -1);
  def_INST_TABW("?? 001 ???", ror, -1);
  def_INST_TABW("?? 100 ???", shl, -1);
  def_INST_TABW("?? 101 ???", shr, -1);
  def_INST_TABW("?? 111 ???", sar, -1);
  return EXEC_ID_inv;
}

def_THelper(gp3) {
  def_INST_IDTABW("?? 000 ???", test_I, test, s->isa.width);
  def_INST_TABW  ("?? 010 ???", not, -1);
  def_INST_TABW  ("?? 011 ???", neg, -1);
  def_INST_TABW  ("?? 100 ???", mul, -1);
  def_INST_TABW  ("?? 101 ???", imul1, -1);
  def_INST_TABW  ("?? 110 ???", div, -1);
  def_INST_TABW  ("?? 111 ???", idiv, -1);
  return EXEC_ID_inv;
}

def_THelper(gp4) {
  def_INST_TABW("?? 000 ???", inc, -1);
  def_INST_TABW("?? 001 ???", dec, -1);
  return EXEC_ID_inv;
}
...
def_THelper(gp5) {
  def_INST_TABW("?? 000 ???", inc, -1);
  def_INST_TABW("?? 001 ???", dec, -1);
  def_INST_TABW("?? 010 ???", call_E, -1);
  def_INST_TABW("?? 100 ???", jmp_E, -1);
  def_INST_TABW("?? 110 ???", push, -1);
  return EXEC_ID_inv;
}

def_THelper(gp6) {
  def_INST_TABW("?? 011 ???", ltr, -1);
  return EXEC_ID_inv;
}

def_THelper(gp7) {
  def_INST_TABW("?? 010 ???", lgdt, -1);
  def_INST_TABW("?? 011 ???", lidt, -1);
  return EXEC_ID_inv;
}

def_THelper(_2byte_esc) {
  x86_inst_fetch(s, 1, true);
  s->isa.opcode = get_inst(s) | 0x100;

  def_hex_INST_IDTABW("00",    E, gp6, 2);
  def_hex_INST_IDTABW("01",    E, gp7, 4);
  def_hex_INST_IDTABW("20",  G2E, mov_cr2r, 4);
  def_hex_INST_IDTABW("22",  E2G, mov_r2cr, 4);
  def_hex_INST_TAB   ("31",       rdtsc);
  def_INST_IDTAB ("0100 ????",  E2G, cmovcc);
  def_hex_INST_TAB   ("6f",       sse_0x6f);
  def_hex_INST_IDTAB ("73", Ib2E, sse_0x73);
  def_hex_INST_TAB   ("7e",       sse_0x7e);
  def_INST_IDTABW("1000 ????",    J, jcc, 4);
  def_INST_IDTABW("1001 ????",    E, setcc, 1);
  def_hex_INST_TAB   ("a2",       cpuid);
  def_hex_INST_IDTAB ("a3",  G2E, bt);
  def_hex_INST_IDTAB ("a4",Ib_G2E,shld);
  def_hex_INST_IDTAB ("a5",cl_G2E,shld);
  def_hex_INST_IDTAB ("ab",  G2E, bts);
  def_hex_INST_IDTAB ("ac",Ib_G2E,shrd);
  def_hex_INST_IDTAB ("ad",cl_G2E,shrd);
  def_hex_INST_IDTAB ("af",  E2G, imul2);
  def_hex_INST_IDTAB ("b1",  G2E, cmpxchg);
  def_hex_INST_IDTAB ("b6", Eb2G, movzb);
  def_hex_INST_IDTABW("b7", Ew2G, movzw, 4);
  def_hex_INST_IDTAB ("bc",  E2G, bsf);
  def_hex_INST_IDTAB ("bd",  E2G, bsr);
  def_hex_INST_IDTAB ("be", Eb2G, movsb);
  def_hex_INST_IDTABW("bf", Ew2G, movsw, 4);
  def_hex_INST_IDTAB ("c1",  G2E, xadd);
  def_INST_IDTABW("1100 1???", r, bswap, 4);
  def_hex_INST_TAB   ("d6",       sse_0xd6);
  def_hex_INST_TAB   ("ef",       sse_0xef);
  return EXEC_ID_inv;
}
...
def_THelper(main) {
  x86_inst_fetch(s, 1, true);
  s->isa.opcode = get_inst(s);

  def_hex_INST_IDTABW("00",  G2E, add, 1);
  def_hex_INST_IDTAB ("01",  G2E, add);
  def_hex_INST_IDTABW("02",  E2G, add, 1);
  def_hex_INST_IDTAB ("03",  E2G, add);
  def_hex_INST_IDTAB ("05",  I2a, add);
  def_hex_INST_IDTABW("08",  G2E, or, 1);
  def_hex_INST_IDTAB ("09",  G2E, or);
  def_hex_INST_IDTABW("0a",  E2G, or, 1);
  def_hex_INST_IDTAB ("0b",  E2G, or);
  def_hex_INST_IDTABW("0c",  I2a, or, 1);
  def_hex_INST_IDTAB ("0d",  I2a, or);
  def_hex_INST_TAB   ("0f",       _2byte_esc);
  def_hex_INST_IDTABW("10",  G2E, adc, 1);
  def_hex_INST_IDTAB ("11",  G2E, adc);
  def_hex_INST_IDTAB ("13",  E2G, adc);
  def_hex_INST_IDTABW("18",  G2E, sbb, 1);
  def_hex_INST_IDTAB ("19",  G2E, sbb);
  def_hex_INST_IDTAB ("1b",  E2G, sbb);
  def_hex_INST_IDTABW("1c",  I2a, sbb, 1);
  def_hex_INST_IDTABW("20",  G2E, and, 1);
  def_hex_INST_IDTAB ("21",  G2E, and);
  def_hex_INST_IDTABW("22",  E2G, and, 1);
  def_hex_INST_IDTAB ("23",  E2G, and);
  def_hex_INST_IDTABW("24",  I2a, and, 1);
  def_hex_INST_IDTAB ("25",  I2a, and);
  def_hex_INST_IDTABW("28",  G2E, sub, 1);
  def_hex_INST_IDTAB ("29",  G2E, sub);
  def_hex_INST_IDTABW("2a",  E2G, sub, 1);
  def_hex_INST_IDTAB ("2b",  E2G, sub);
  def_hex_INST_IDTABW("2c",  I2a, sub, 1);
  def_hex_INST_IDTAB ("2d",  I2a, sub);
  def_hex_INST_IDTABW("30",  G2E, xor, 1);
  def_hex_INST_IDTAB ("31",  G2E, xor);
  def_hex_INST_IDTABW("32",  E2G, xor, 1);
  def_hex_INST_IDTAB ("33",  E2G, xor);
  def_hex_INST_IDTABW("34",  I2a, xor, 1);
  def_hex_INST_IDTAB ("35",  I2a, xor);
  def_hex_INST_IDTABW("38",  G2E, cmp, 1);
  def_hex_INST_IDTAB ("39",  G2E, cmp);
  def_hex_INST_IDTABW("3a",  E2G, cmp, 1);
  def_hex_INST_IDTAB ("3b",  E2G, cmp);
  def_hex_INST_IDTABW("3c",  I2a, cmp, 1);
  def_hex_INST_IDTAB ("3d",  I2a, cmp);
  def_INST_IDTAB ("0100 0???",    r, inc);
  def_INST_IDTAB ("0100 1???",    r, dec);
  def_INST_IDTAB ("0101 0???",    r, push);
  def_INST_IDTAB ("0101 1???",    r, pop);
  def_hex_INST_TAB   ("60",       pusha);
  def_hex_INST_TAB   ("61",       popa);
  def_hex_INST_TAB   ("65",       gs);
  def_hex_INST_TAB   ("66",       operand_size);
  def_hex_INST_IDTAB ("68",    I, push);
  def_hex_INST_IDTAB ("69",I_E2G, imul3);
  def_hex_INST_IDTABW("6a",   SI, push, 1);
  def_hex_INST_IDTAB ("6b",SI_E2G,imul3);
  def_INST_IDTABW("0111 ????",    J, jcc, 1);
  def_hex_INST_IDTABW("80",  I2E, gp1, 1);
  def_hex_INST_IDTAB ("81",  I2E, gp1);
  def_hex_INST_IDTAB ("83", SI2E, gp1);
  def_hex_INST_IDTABW("84",  G2E, test, 1);
  def_hex_INST_IDTAB ("85",  G2E, test);
  def_hex_INST_IDTABW("86",  G2E, xchg, 1);
  def_hex_INST_IDTAB ("87",  G2E, xchg);
  def_hex_INST_IDTABW("88",  G2E, mov, 1);
  def_hex_INST_IDTAB ("89",  G2E, mov);
  def_hex_INST_IDTABW("8a",  E2G, mov, 1);
  def_hex_INST_IDTAB ("8b",  E2G, mov);
  def_hex_INST_IDTABW("8d",  E2G, lea, 4);
  def_hex_INST_IDTABW("8e",  E2G, mov_rm2sreg, 2);
  def_hex_INST_TAB   ("90",       nop);
  def_INST_IDTAB ("1001 0???",  a2r, xchg);
  def_hex_INST_TAB   ("98",       cwtl);
  def_hex_INST_TAB   ("99",       cltd);
  def_hex_INST_TAB   ("9b",       fwait);
  def_hex_INST_TAB   ("9c",       pushf);
  def_hex_INST_TAB   ("9d",       popf);
  def_hex_INST_TAB   ("9e",       sahf);
  def_hex_INST_IDTABW("a0",  O2a, mov, 1);
  def_hex_INST_IDTAB ("a1",  O2a, mov);
  def_hex_INST_IDTABW("a2",  a2O, mov, 1);
  def_hex_INST_IDTAB ("a3",  a2O, mov);

  if (s->isa.rep_flags == PREFIX_REP) {
    def_hex_INST_TABW  ("a4", rep_movs, 1);
    def_hex_INST_TAB   ("a5", rep_movs);
    def_hex_INST_TABW  ("a6", repz_cmps, 1);
    def_hex_INST_IDTABW("aa", aSrc, rep_stos, 1);
    def_hex_INST_IDTAB ("ab", aSrc, rep_stos);
  } else if (s->isa.rep_flags == PREFIX_REPNZ) {
    def_hex_INST_IDTABW("ae", aSrc, repnz_scas, 1);
  }

  def_hex_INST_TABW  ("a4",       movs, 1);
  def_hex_INST_TAB   ("a5",       movs);
  def_hex_INST_IDTABW("a8",  I2a, test, 1);
  def_hex_INST_IDTAB ("a9",  I2a, test);
  def_hex_INST_IDTABW("aa", aSrc, stos, 1);
  def_INST_IDTABW("1011 0???",  I2r, mov, 1);
  def_INST_IDTAB ("1011 1???",  I2r, mov);
  def_hex_INST_IDTABW("c0", Ib2E, gp2, 1);
  def_hex_INST_IDTAB ("c1", Ib2E, gp2);
  def_hex_INST_IDTABW("c2",    I, ret_imm, 2);
  def_hex_INST_TAB   ("c3",       ret);
  def_hex_INST_IDTABW("c6",  I2E, mov, 1);
  def_hex_INST_IDTAB ("c7",  I2E, mov);
  def_hex_INST_TAB   ("c9",       leave);
  def_hex_INST_TAB   ("cc",       nemu_trap);
  def_hex_INST_IDTABW("cd",    I, _int, 1);
  def_hex_INST_TAB   ("cf",       iret);
  def_hex_INST_IDTABW("d0",  1_E, gp2, 1);
  def_hex_INST_IDTAB ("d1",  1_E, gp2);
  def_hex_INST_IDTABW("d2", cl2E, gp2, 1);
  def_hex_INST_IDTAB ("d3", cl2E, gp2);
  def_hex_INST_TAB   ("d8",       fpu_d8);
  def_hex_INST_TAB   ("d9",       fpu_d9);
  def_hex_INST_TAB   ("da",       fpu_da);
  def_hex_INST_TAB   ("db",       fpu_db);
  def_hex_INST_TAB   ("dc",       fpu_dc);
  def_hex_INST_TAB   ("dd",       fpu_dd);
  def_hex_INST_TAB   ("de",       fpu_de);
  def_hex_INST_TAB   ("df",       fpu_df);
  def_hex_INST_IDTABW("e3",    J, jecxz, 1);
  def_hex_INST_IDTABW("e8",    J, call, 4);
  def_hex_INST_IDTABW("e9",    J,  jmp, 4);
  def_hex_INST_IDTABW("eb",    J,  jmp, 1);
  def_hex_INST_IDTAB ("ed", dx2a, in);
  def_hex_INST_IDTABW("ee", a2dx, out, 1);
  def_hex_INST_IDTAB ("ef", a2dx, out);
  def_hex_INST_TAB   ("f0",       lock);
  def_hex_INST_IDTABW("f6",    E, gp3, 1);
  def_hex_INST_IDTAB ("f7",    E, gp3);
  def_hex_INST_TAB   ("f2",       repnz);
  def_hex_INST_TAB   ("f3",       rep);
  def_hex_INST_TAB   ("f8",       clc);
  def_hex_INST_TAB   ("f9",       stc);
  def_hex_INST_TAB   ("fc",       cld);
  def_hex_INST_TAB   ("fd",       std);
  def_hex_INST_IDTABW("fe",    E, gp4, 1);
  def_hex_INST_IDTAB ("ff",    E, gp5);
  return table_inv(s);
}

int isa_exec_once(Decode *s) {
  int idx = EXEC_ID_inv;
  s->isa.p_inst = s->isa.inst;
  ...
  switch (idx) {
    ...
    case EXEC_ID_repnz_scas:
      s->jnpc = s->pc; s->type = INST_TYPE_B; break;
  ...
  if (s->type != INST_TYPE_N) { // the end of a basic block
    if (s - bb_start == bb_idx) {
      // now scan and update `flag_def`
      Decode *p;
      uint32_t use = F_ALL;
      //uint32_t use = (idx == EXEC_ID_call || s->type == INST_TYPE_I ? 0 : F_ALL); //s->isa.flag_use;
