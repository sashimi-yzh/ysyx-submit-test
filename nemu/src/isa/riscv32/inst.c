...
enum {
  TYPE_I, TYPE_U, TYPE_S, TYPE_J, TYPE_R, TYPE_B, TYPE_RI,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, { R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1; IFDEF(CONFIG_FTRACE, {
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   ,RI, R(rd) = src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   ,RI, R(rd) = (sword_t)src1 >> imm);
