...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, {uint32_t shamt = BITS(imm,4,0);R(rd) = src1 << shamt;});
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, {uint32_t shamt = BITS(imm,4,0);R(rd) = (uint32_t)src1 >> shamt;});
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, {uint32_t shamt = BITS(imm,4,0);R(rd) = (int32_t)src1 >> shamt;});
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", li     , I, R(rd)=src1+imm);
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd)=src1+SEXT(imm,12));//mv拓展为addi rd, rs1, 0 
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd)=(uint32_t)src1<(uint32_t)imm ? 1:0);//seqz实际被扩展为sltiu rd, rs1, 1
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd)=(int32_t)src1<(int32_t)imm ? 1:0);
  INSTPAT("0000000 00001 00000 000 00000 11001 11", ret    , I, s->dnpc = (src1 + imm) & ~1); // ret 是 jalr x0, 0(x1)
