...
#define def_ld_template(name, rtl_inst, width, mmu_mode) \
  def_EHelper(name) { concat(rtl_, rtl_inst) (s, ddest, dsrc1, id_src2->imm, width, mmu_mode); }
#define def_st_template(name, rtl_inst, width, mmu_mode) \
  def_EHelper(name) { concat(rtl_, rtl_inst) (s, dsrc2, dsrc1, id_dest->imm, width, mmu_mode); }

#define def_all_ldst_common(suffix, mmu_mode) \
  def_ld_template(concat(lw , suffix), MUXDEF(CONFIG_RV64, lms, lm), 4, mmu_mode) \
...
#define def_all_ldst(suffix, mmu_mode) \
  def_all_ldst_common(suffix, mmu_mode) \
...
#else
#define def_all_ldst def_all_ldst_common
#endif
