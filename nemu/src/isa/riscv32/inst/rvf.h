...
#define check_rm(s) IFDEF(CONFIG_RT_CHECK, Assert(INST_FP_RM(s) == 0b111, "s->pc = " FMT_WORD, s->pc));
...
def_EHelper(fcvt_s_w_rm) {
  rtl_fpcall(s, FPCALL_SETRM_CONST, NULL, NULL, NULL, INST_FP_RM(s));
  rtl_fcvt_i32_to_f32(s, ddest, dsrc1);
