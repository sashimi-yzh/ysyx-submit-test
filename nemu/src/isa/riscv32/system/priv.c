...
static void csr_write(word_t *dest, word_t src) {
  ...
  if (is_write(fflags) || is_write(frm) || is_write(fcsr)) {
    ...
    rtl_fpcall(NULL, FPCALL_SETRM_CONST, NULL, NULL, NULL, 0b111);
...
static word_t priv_inst(uint32_t op, const rtlreg_t *src) {
  switch (op) {
...
void isa_hostcall(uint32_t id, rtlreg_t *dest,
  ...
  switch (id) {
    ...
    case HOSTCALL_PRIV: ret = priv_inst(imm, src1); break;
