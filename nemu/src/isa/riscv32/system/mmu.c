...
// Sv39 and Sv32 page walk
#define PTW_LEVEL MUXDEF(CONFIG_RV64, 3,  2)
#define PTE_SIZE sizeof(word_t)
#define VPN_LEN MUXDEF(CONFIG_RV64, 9, 10)
#define VPNMASK ((1 << (VPN_LEN)) - 1)

static uintptr_t VPNiSHFT(int i) {
  return (PGSHFT) + VPN_LEN * i;
}
...
static int update_mmu_state_internal(bool ifetch) {
  uint32_t mode = (mstatus->mprv && (!ifetch) ? mstatus->mpp : cpu.mode);
  if (mode < MODE_M) {
    IFDEF(CONFIG_RV64, assert(satp->mode == 0 || satp->mode == 8));
    if (satp->mode == MUXDEF(CONFIG_RV64, 8, 1)) return MMU_TRANSLATE;
  }
  return MMU_DIRECT;
}
