...
Decode* tcache_decode(Decode *s) {
  ...
  exec_once(s, thispc); // note that exception may happen!

  if (s->type == INST_TYPE_N) {
    ...
    switch (s->type) {
      case INST_TYPE_J: tcache_bb_fetch(s, true, s->jnpc); break;
      case INST_TYPE_B:
        tcache_bb_fetch(s, true, s->jnpc);
        tcache_bb_fetch(s, false, s->snpc + MUXDEF(CONFIG_ISA_mips32, 4, 0));
        break;
      case INST_TYPE_I: s->tnext = s->ntnext = s; break; // update dynamically
      default: assert(0);
    }
