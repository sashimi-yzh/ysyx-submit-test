...
#ifndef __ICS_EXPORT
  IFDEF(CONFIG_IQUEUE, iqueue_commit(_this->pc, (void *)&_this->isa.inst, _this->snpc - _this->pc));
...
IFNDEF(CONFIG_TARGET_SHARE, static uint64_t g_nr_guest_inst_end = 0);
...
#define rtl_j(s, target) do { \
  ...
  IFDEF(CONFIG_ICOUNT_BASIC_BLOCK, g_nr_guest_inst += s->idx_in_bb); \
  s = s->tnext; \
  goto end_of_bb; \
} while (0)
...
#define rtl_priv_next(s) do { \
  if (g_sys_state_flag) { \
    IFDEF(CONFIG_ICOUNT_PRECISE, g_nr_guest_inst ++); \
...
static void execute(int64_t n) {
  static const void* local_exec_table[TOTAL_INST] = {
    MAP(INST_LIST, FILL_EXEC_TABLE)
  };
...
#include <isa-exec.h>
#define FILL_EXEC_TABLE(name) [concat(EXEC_ID_, name)] = concat(exec_, name),
static const void* g_exec_table[TOTAL_INST] = {
  MAP(INST_LIST, FILL_EXEC_TABLE)
};
#endif
#endif
...
#else
    IFDEF(CONFIG_DEVICE, if ((n & 0xffff) == 0) device_update());
...
#if !defined(CONFIG_TARGET_SHARE) || defined(__ICS_EXPORT)
...
#ifdef CONFIG_PERF_OPT
  g_nr_guest_inst_end = g_nr_guest_inst + n;
  int cause = NEMU_EXEC_RUNNING;
#ifndef CONFIG_TARGET_AM
  if ((cause = setjmp(jbuf_exec))) {
    IFDEF(CONFIG_ICOUNT_BASIC_BLOCK, g_nr_guest_inst += prev_s->idx_in_bb - 1);
    update_global();
  }
#endif

  while (nemu_state.state == NEMU_RUNNING &&
      MUXDEF(CONFIG_ICOUNT_DISABLE, true, g_nr_guest_inst < g_nr_guest_inst_end)) {
    IFDEF(CONFIG_DEVICE, device_update());
    ...
    uint32_t nremain = g_nr_guest_inst_end - g_nr_guest_inst;
