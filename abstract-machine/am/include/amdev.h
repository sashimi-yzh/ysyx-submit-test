...
AM_DEVREG( 5, TIMER_RTC,    RD, int year, month, day, hour, minute, second);//时钟
AM_DEVREG( 6, TIMER_UPTIME, RD, uint64_t us);//时钟
AM_DEVREG( 7, INPUT_CONFIG, RD, bool present);
AM_DEVREG( 8, INPUT_KEYBRD, RD, bool keydown; int keycode);//AM键盘控制器
AM_DEVREG( 9, GPU_CONFIG,   RD, bool present, has_accel; int width, height, vmemsz);//AM显示控制器信息, 可读出屏幕大小信息
AM_DEVREG(10, GPU_STATUS,   RD, bool ready);
AM_DEVREG(11, GPU_FBDRAW,   WR, int x, y; void *pixels; int w, h; bool sync);//AM帧缓冲控制器, 可写入绘图信息
