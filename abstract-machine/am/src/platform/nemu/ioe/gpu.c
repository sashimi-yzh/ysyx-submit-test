...
void __am_gpu_config(AM_GPU_CONFIG_T *cfg) {  //显示控制器信息
	uint32_t screen = inl(VGACTL_ADDR);      //高16位为width, 低16位为height
	uint32_t h = screen & 0x0000ffff;
...
void __am_gpu_fbdraw(AM_GPU_FBDRAW_T *ctl) {   //帧缓冲控制器
	...
	uint32_t screen_w = inl(VGACTL_ADDR) >> 16;  //行优先
	uint32_t *fb_addr = (uint32_t *)(uintptr_t)FB_ADDR;   //帧缓冲区地址
