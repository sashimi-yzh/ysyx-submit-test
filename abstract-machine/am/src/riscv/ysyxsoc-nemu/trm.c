...
void __attribute__((section(".text.bootloader2"))) bootloader2(){
	extern char _text_start;  // VMA中.text段的起始地址 存储在_text_start地址(0x80000000)处的一个字节的数据
	extern char _text_LMA_addr;// LMA中.text段的起始地址
	extern char _bss_start;  
	size_t len =  (size_t)&_bss_start - (size_t)&_text_start; //&_text_start:char*（指向char的指针）,_text_start的地址
	//4字节4字节转移
	if (len != 0){ //text,rodata,data一块转移
		//memcpy((void*)text_start, (const void*)text_LMA_addr, len); 
		uint32_t *out = (void*)&_text_start;  //将_text_start的地址转换为无类型指针
		const uint32_t *in = (const void*)&_text_LMA_addr;	
		size_t word_count = len / 4;
		for(size_t i = 0; i < word_count; i++){
			out[i] = in[i];
		}
...
void _trm_init() {
	//bootloader();//将指令，rodata,全局变量从LMA到VMA
	UART_init();// 初始化UART串口
