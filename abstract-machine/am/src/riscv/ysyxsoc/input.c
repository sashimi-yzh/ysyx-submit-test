...
static int keylut[140] = {
  ...
  [0x05] = AM_KEY_F1,
  [0x06] = AM_KEY_F2,                   [0x04] = AM_KEY_F3,
  [0x0C] = AM_KEY_F4,                   [0x03] = AM_KEY_F5,
  [0x0B] = AM_KEY_F6,                   [0x83] = AM_KEY_F7,
  [0x0A] = AM_KEY_F8,                   [0x01] = AM_KEY_F9,
  [0x09] = AM_KEY_F10,                  [0x78] = AM_KEY_F11,
  [0x07] = AM_KEY_F12,
};

volatile int old_code = 0;
volatile int cur_code = 0;

void __am_input_keybrd(AM_INPUT_KEYBRD_T *kbd) {
  volatile int code = inb(PS2_BASE) & 0xff;
  if (code) {
    old_code = cur_code;
    cur_code = code;
    if (cur_code == 0xf0) {
    ...
    if (old_code == 0xf0) {
      kbd->keydown = false;
      kbd->keycode = keylut[cur_code & 0xff];
    } else {
      kbd->keydown = true;
      kbd->keycode = keylut[cur_code & 0xff];
    }
