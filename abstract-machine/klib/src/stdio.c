...
static void _putch(char c, int n, format_t *f) {
  while (n --) {
...
print_num:;
        ...
        if (!f.pad.is_right && f.pad.ch == ' ') { _putch(f.pad.ch, width_pad_len, &f); }
        if (sign_len) _putch(f.sign.ch, 1, &f);
        if (!f.pad.is_right && f.pad.ch == '0') { _putch(f.pad.ch, width_pad_len, &f); }
        _putch('0', prec_pad0_len, &f);
        __putstr(p_digit, digit_len, &f);
        if (f.pad.is_right) { _putch(f.pad.ch, width_pad_len, &f); }
        break;
